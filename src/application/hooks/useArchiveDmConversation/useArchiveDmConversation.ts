...
export function useArchiveDmConversation(): UseMutationResult<
  void,
  Error,
  string,
  Context
> {
  ...
  return useMutation<void, Error, string, Context>({
    mutationKey: ['dm', 'conversations', 'archive'],
    mutationFn: (peerId) => dmApi.archiveDmConversation(peerId),
    onMutate: async (peerId) => {
      ...
      setDmArchivedInCaches(queryClient, peerId, true);
